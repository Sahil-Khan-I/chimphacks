...
export default function Home() {
  ...
  return (
    <motion.div 
      ...
      className={`relative flex flex-col items-center justify-center min-h-screen p-8 transition-colors duration-500 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}
    >
      <header className="flex flex-col items-center gap-4 w-full max-w-4xl">
        <div className="w-full flex justify-end">
          <motion.button
            className={`p-2 rounded-full shadow-lg ${isDarkMode ? 'bg-gray-800 text-yellow-400' : 'bg-gray-200 text-gray-800'}`}
      ...
      <main className="flex flex-col gap-8 items-center w-full px-4">
        ...
        <div className="flex gap-4 mt-6">
          {['Join Us', 'Learn More'].map((text, index) => (
            <a
              key={index}
              ...
              href={index === 0 ? "https://github.com/chimphacks" : "https://hackclub.com"}
              target="_blank"
      ...
      <footer className="flex gap-6 flex-wrap items-center justify-center">
        {[{ href: "https://www.linkedin.com/in/sahil-khan-indie/", icon: <FaLinkedin aria-hidden />, label: "LinkedIn" },
          { href: "https://solobusinessbrief.beehiiv.com/", icon: <FaEnvelope aria-hidden />, label: "Newsletter" }].map(({ href, icon, label }, index) => (
          <motion.a
            key={index}
            whileHover={{ scale: 1.05 }}
            className={`flex items-center gap-2 hover:underline hover:underline-offset-4 ${isDarkMode ? 'text-gray-300 hover:text-yellow-400' : 'text-gray-800 hover:text-blue-600'}`}
            href={href}
            target="_blank"
            rel="noopener noreferrer"
          >
